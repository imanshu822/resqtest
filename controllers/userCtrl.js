...
const createResource = asyncHandler(async (req, res) => {
  const { resName, resQuantity } = req.body;
  try {
    // Check if the resource already exists for the user
    const user = await User.findOne({ "resources.name": resName });
    if (user) {
      // Resource already exists for the user, update its quantity
      const resource = user.resources.find((r) => r.name === resName);
      resource.quantity += resQuantity;
      await user.save();
      res.json(user);
    } else {
      // Resource doesn't exist, create a new one
      const newUser = await User.findByIdAndUpdate(
        req.user.id, // Assuming you have the authenticated user's ID in req.user
        {
          $push: {
            resources: { resName: resName, resQuantity: resQuantity },
          },
        },
        { new: true }
      );
